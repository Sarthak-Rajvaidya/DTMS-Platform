document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.checkWildfireQuiz = function() {
        ...
        let score = 0;
        if (q1 && q1.value === '90') score++;
        if (q2 && q2.value === 'wind') score++;
        if (q3 && q3.value === 'leave') score++;
        if (q4 && q4.value === '5ft') score++;
        if (q5 && q5.value === 'n95') score++;
        
        let resultDiv = document.getElementById('quizResult');
        if (score === 5) {
            resultDiv.innerHTML = '<div class="alert alert-success">🎉 Perfect score! You\'re wildfire-ready! 🔥✅</div>';
            markSectionViewed('section8');
        } else {
            resultDiv.innerHTML = `<div class="alert alert-warning">📚 Score: ${score}/5. ${score < 3 ? 'Please review sections 3-5 again!' : 'Good effort! Review a bit more to master wildfire safety.'}</div>`;
        }
    }
    ...
    console.log('Wildfire Safety Module Initialized with Persistence');
});